...
counts_1 <- all_trips_v2 %>% 
  mutate(weekday = wday(started_at, label = TRUE)) %>% 
  group_by(member_casual, weekday) %>% 
  summarise(number_of_rides = n()
            ,average_duration = mean(ride_length)) %>% 
  arrange(member_casual, weekday)
write.csv(counts_1, file = 'tot_ride_count.csv')

print(counts_1)

nrow(all_trips_v2)

counts_2<- all_trips_v2 %>% 
  mutate(monthname = ordered(months(started_at, abbreviate = TRUE),levels=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul","Aug","Sep","Oct","Nov","Dec"))) %>% 
  group_by(member_casual, monthname) %>% 
  summarise(number_of_rides = n()
            ,average_duration = mean(ride_length)) %>% 
  arrange(member_casual, monthname)  

write.csv(counts_2, file = 'month_ride_count.csv')

ggplotly(all_trips_v2 %>% 
           mutate(monthname = ordered(months(started_at, abbreviate = TRUE),levels=c("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul","Aug","Sep","Oct","Nov","Dec"))) %>% 
           group_by(member_casual, monthname) %>% 
           summarise(number_of_rides = n()
                     ,average_duration = mean(ride_length)) %>% 
           arrange(member_casual, monthname)  %>% 
           ggplot(aes(x = monthname, y = number_of_rides, fill = member_casual, text = paste("Number of rides:",number_of_rides))) +
           geom_col(position = "dodge"), tooltip="text")
